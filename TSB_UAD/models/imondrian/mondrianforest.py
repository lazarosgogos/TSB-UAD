...
from .mondrianforest_utils import add_stuff_2_settings
from .mondrianforest_utils import compute_gaussian_logpdf
from .mondrianforest_utils import compute_left_right_statistics
from .mondrianforest_utils import Forest
from .mondrianforest_utils import get_filename_mf
from .mondrianforest_utils import init_prediction_tree
from .mondrianforest_utils import init_update_posterior_node_incremental
from .mondrianforest_utils import parser_add_common_options
from .mondrianforest_utils import parser_add_mf_options
from .mondrianforest_utils import parser_check_common_options
from .mondrianforest_utils import parser_check_mf_options
from .mondrianforest_utils import update_posterior_node_incremental
from .utils import approx_avg_path_length
from .utils import check_if_one
from .utils import check_if_zero
from .utils import logsumexp_array
from .utils import multiply_gaussians
from .utils import sample_multinomial_scores
from .utils import sigmoid

from .mondrianforest_utils import precompute_minimal
from .mondrianforest_utils import reset_random_seed
...
class MondrianTree:
    ...
    def extend_mondrian_block(self, node_id, train_ids_new, data, settings, param, cache):
        ...
        # print('--TRAIN IDS NEW', train_ids_new)
        if len(train_ids_new) == 0:
            if settings.debug:
...
class MondrianForest(Forest):
    ...
    def score_samples(self, X):
        ...
        return self._score_samples(X, self._settings)
    
    def _score_samples(self, X, settings):
        ...
        return -self._compute_chunked_score_samples(X, settings)


    
    def _compute_chunked_score_samples(self, X, settings):
        n_samples = sklearn.utils.validation._num_samples(X)
        if self._max_features == len(X):
        ...
        for sl in slices:
            ...
            # FIXME subsample_features instead of self._max_features
            # to effectively use subsamples ! instead, this hacky way passes the 
            # max features varialbe straight in the computation
            scores[sl] = self._compute_score_samples(X[sl], settings)
            

        return scores
    
    def _compute_score_samples(self, X, settings, subsampling_size=None, ):
        return self.get_anomaly_scores(X, settings, subsampling_size=subsampling_size)[0]
    
    def get_anomaly_scores(self, test_data, settings, subsampling_size=None):
        if subsampling_size is not None:
            approx_avg_depth = approx_avg_path_length(subsampling_size)
        else:
            if settings.optype == 'class':
                sample_size = self.forest[0].counts[self.forest[0].root].sum()
            else:
                sample_size = self.forest[0].n_points[self.forest[0].root]
            approx_avg_depth = approx_avg_path_length(sample_size)
        ...
        for t in self.forest:
            for i, x in enumerate(test_data):
                total_depths[i] += t.path_length(x, settings)

        avg_depths = total_depths / len(self.forest)
        with np.errstate(divide='ignore', invalid='ignore'):
            scores = np.where(approx_avg_depth == 0, 1.0, 2 ** (-avg_depths / approx_avg_depth))
        scores_shifted = -scores + 0.5
        return scores, scores_shifted


    def _get_anomaly_scores(self, test_data, settings, subsampling_size=None):
